 ...
 * @copyright   2020 Janez Paternoster, hustGs
 *
...
typedef uint16_t           oChar_t;
typedef uint16_t           domain_t;


/* Access to received CAN message */
#define CO_CANrxMsg_readIdent(msg) ( (uint16_t) ( *((uint32_t *)(msg)) ) )
#define CO_CANrxMsg_readDLC(msg)   ( *( (uint8_t *)(msg) + 2)  )
#define CO_CANrxMsg_readData(msg)   ((uint8_t *)(msg)  + 4)
...
typedef struct {
    ...
    volatile bool_t useCANrxFilters;//Value different than zero indicates, that CAN module hardware filters are used for CAN reception.
    volatile bool_t bufferInhibitFlag;//If flag is true, then message in transmit buffer is synchronous PDO message, which will be aborted, if CO_clearPendingSyncPDOs() function will be called by application
